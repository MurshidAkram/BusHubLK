...
const UNION_NOTIFICATIONS_CTE = `
  WITH union_notifications AS (
    ...
    COALESCE(dom.region_id, dm.region_id, $2) AS region_id,
    ...
    WHERE c.channel_type = 'announcement'
      AND r.role_name IN ('ceo', 'dgm_operations', 'dgm_technical', 'admin', 'depot_operations', 'depot_manager')
      AND (
        r.role_name IN ('ceo', 'dgm_operations', 'dgm_technical', 'admin')
        OR (r.role_name = 'depot_operations' AND dom.region_id = $2)
...
class RegionalOperationsNotificationModel {
    static async getNotifications({ userId, regionId, includeRead = false, limit = 100, offset = 0 }) {
      ...
      SELECT
        ...
        un.meta,
        ronr.read_at,
        COALESCE(ronr.is_read, false) AS is_read
      FROM union_notifications un
      LEFT JOIN regional_operations_notifications_read ronr
        ON ronr.user_id = $1
       AND ronr.source_type = un.source_type
       AND ronr.source_id = un.source_id
      WHERE $3::boolean OR ronr.id IS NULL
    ...
    static async markAsRead({ userId, sourceType, sourceId, regionId }) {
        ...
        const query = `
      INSERT INTO regional_operations_notifications_read (user_id, source_type, source_id, region_id, is_read, read_at)
...
module.exports = RegionalOperationsNotificationModel;
