...
const Passenger = {
  ...
  addEmergencyContact: async (passengerId, contactName, contactPhone, relationship = 'N/A', email = 'N/A', isPrimary = false) => {
    const query = {
  ...
  const query = {
    ...
    values: [name, phone, relationship, email, isPrimary, contactId],
  };
  const { rows } = await pool.query(query);
  return rows[0];
},
...
deleteEmergencyContact: async (contactId) => {
  const query = {
    text: `DELETE FROM emergency_contacts WHERE id=$1 RETURNING *`,
    values: [contactId],
  };
  const { rows } = await pool.query(query);
  return rows[0];
  ...
  // You can add functions for creating alerts here later
};
