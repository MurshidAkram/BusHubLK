...
import { Clock, AlertTriangle, CheckCircle, XCircle, MessageSquare, MapPin, User, Filter, Search, Bell, Car, FileText, Send, ArrowUp, CheckSquare, ChevronUp, ChevronDown } from 'lucide-react';
...
const Rtoissuetracker = () => {
  ...
  const [emergencyReports, setEmergencyReports] = useState<EmergencyReport[]>([
    {
      ...
      chatHistory: [
        ...
        { sender: 'depot', message: 'Escalating to RTO for further assessment', time: '09:30' },
      ],
    },
    {
      id: '23',
      ...
      chatHistory: [
        ...
        { sender: 'depot', message: 'Escalating to RTO for engine diagnostics', time: '09:00' },
      ],
    },
  ]);
  ...
  const stats = {
    ...
    escalatedToRTO: emergencyReports.filter((r: EmergencyReport) => r.escalatedToRTO).length,
    ...
    if (chatMessage.trim() && selectedIssue) {
      ...
      const updatedReports = emergencyReports.map((report: EmergencyReport) =>
        report.id === selectedIssue.id
          ? { ...report, chatHistory: [...report.chatHistory, { sender: 'rto', message: chatMessage, time: currentTime }] }
          : report
      );
      setEmergencyReports(updatedReports);
      setSelectedIssue({
        ...selectedIssue,
        chatHistory: [...selectedIssue.chatHistory, { sender: 'rto', message: chatMessage, time: currentTime }],
      });
      setChatMessage('');
    }
  };
      ...
      const updatedReports = emergencyReports.map((report: EmergencyReport) =>
        report.id === selectedIssue.id
          ? {
              ...
              escalatedToDGM: true,
              dgmEscalationReason: escalationReason,
      ...
      <div className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            <h1 className="text-xl font-bold text-gray-900">RTO Technical Dashboard</h1>
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center">
                <div className="ml-3">
                  <p className="text-sm font-medium text-gray-700">RTO Technical Officer</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4 mb-6">
          ...
          <div className="bg-white rounded-lg shadow-sm p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">RTO Cases</p>
                        ...
                        <div className="mt-2 p-2 bg-gray-50 rounded-lg">
                          <div className="max-h-60 overflow-y-auto mb-2 space-y-2">
                            {report.chatHistory.map((chat: { sender: string; message: string; time: string }, index: number) => (
                              <div
                                key={index}
                                className={`p-2 rounded-lg ${chat.sender === 'rto' ? 'bg-blue-100 text-right' : 'bg-gray-200'}`}
                              >
                                <p className="text-sm font-medium">{chat.sender === 'rto' ? 'You' : chat.sender}</p>
              ...
              <div className="space-y-4">
                ...
                {selectedIssue.escalationReason && (
                  <div className="bg-purple-50 p-3 rounded-lg">
                    <p className="text-xs text-purple-600 uppercase tracking-wide mb-1">Escalation Reason (From Depot)</p>
                    <p className="text-sm text-purple-800">{selectedIssue.escalationReason}</p>
                  </div>
                )}

                {selectedIssue.dgmEscalationReason && (
                  <div className="bg-indigo-50 p-3 rounded-lg">
                    <p className="text-xs text-indigo-600 uppercase tracking-wide mb-1">Escalation Reason (To DGM)</p>
                    <p className="text-sm text-indigo-800">{selectedIssue.dgmEscalationReason}</p>
                  </div>
                )}
                ...
                <div className="border-t pt-4">
                  {selectedIssue.status !== 'Resolved' && !selectedIssue.escalatedToDGM && (
                    <div className="flex space-x-2">
                      <button
                      ...
                      <button
                        className="flex-1 bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors flex items-center justify-center space-x-1"
                        onClick={() => setShowEscalateModal(true)}
                      >
                        <ChevronUp className="w-4 h-4" />
                        <span>Escalate to DGM</span>
                      </button>
                    </div>
                  )}
                </div>
        ...
        {/* Escalate to DGM Modal */}
        {showEscalateModal && selectedIssue && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Escalate to DGM Technical</h3>
              <p className="text-sm text-gray-600 mb-4">Provide a reason for escalation to DGM Technical Division:</p>
              <textarea
                value={escalationReason}
                onChange={(e) => setEscalationReason(e.target.value)}
                placeholder="e.g., Complex technical issue requiring DGM approval..."
