...
const Busmanagement: React.FC = () => {
  ...
  const fetchPartChangesForBus = async (busId: string): Promise<PartChange[]> => {
    try {
      ...
      const history = Array.isArray(response.data?.usageHistory)
        ? response.data.usageHistory
        : [];

      return history.map((item: any) => ({
        date: formatDate(item.usage_date),
        part: item.part_name || 'Unknown part',
        quantity: Number(item.quantity_used) || 0,
      }));
    } catch (err) {
      const axiosError = err as AxiosError;
      console.error(`Failed to fetch part changes for bus ${busId}:`, axiosError.message);
      return [];
    }
  };
    ...
    try {
      ...
      if (response.data.buses) {
        const fetchedBuses = await Promise.all(
          response.data.buses.map(async (bus) => {
            const [serviceHistory, partChanges] = await Promise.all([
              fetchServiceHistoryForBus(bus.bus_id),
              fetchPartChangesForBus(bus.bus_id),
            ]);
      ...
      if (axiosError.response) {
        ...
        if (axiosError.response.status === 403) {
            ...
            const responseMessage = (axiosError.response.data as { message?: string })?.message;
            setError(`Failed to fetch buses: ${axiosError.response.status} - ${responseMessage || axiosError.response.statusText}. Please verify the API endpoint with the backend team.`);
        }
  ...
  const filteredBuses = buses.filter((bus) => {
    const registration = (bus.registration_number || '').toLowerCase();
    const model = (bus.model || '').toLowerCase();
    const search = searchTerm.toLowerCase();
    const matchesSearch = registration.includes(search) || model.includes(search);
