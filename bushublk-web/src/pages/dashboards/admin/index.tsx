    ...
    const fetchData = async () => {
      ...
      try {
        const [usersRes, busesRes, depotsRes] = await Promise.all([
          ...
          fetch(`${import.meta.env.VITE_API_URL}/api/buses`, { headers: { 'Authorization': `Bearer ${context?.token}` } }),
          fetch(`${import.meta.env.VITE_API_URL}/api/depots`, { headers: { 'Authorization': `Bearer ${context?.token}` } })
        ]);
