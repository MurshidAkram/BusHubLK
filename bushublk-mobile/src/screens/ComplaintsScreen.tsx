...
export default function ComplaintsScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('≡ƒÜÇ Starting complaint submission to:', submitUrl);
      console.log('≡ƒôª FormData contents:', {
      ...
      console.log('ΓÅ▒∩╕Å Starting fetch with 25s timeout...');
      const response = await Promise.race([fetchPromise, timeoutPromise]) as Response;
      console.log('Γ£à Response received:', response.status, response.statusText);
      ...
      if (error.message && error.message.includes('timeout')) {
        console.log('≡ƒòÉ Complaint submission timed out after 25 seconds');
        Alert.alert(
          "Connection Timeout", 
          "Your complaint submission is taking longer than expected. This might be due to:\n\nΓÇó Slow internet connection\nΓÇó Server overload\nΓÇó Large image file\n\nPlease try again or contact support."
        );
      } else if (error.message && error.message.includes('Network request failed')) {
        Alert.alert(
          "Network Error", 
          "Unable to connect to the server. Please check:\n\nΓÇó Your internet connection\nΓÇó Server availability\nΓÇó Try again in a moment"
        );
      } else {
